...
import { initSocket } from "@/lib/socket"; // 🟦 sizning socket wrapperingiz
...
export default function DastavkaPage() {
  ...
  useEffect(() => {
    const socket = initSocket("dastavka");
    ...
    socket.on("all_orders", (data: Order[]) => {
      const filtered = data.filter(
        (o) => o.OrderType === "Dastavka" && o.status === "in_progress"
      );
      setOrders(filtered);
    });
    ...
    socket.on("order_updated", (updated: Order) => {
      if (updated.OrderType !== "Dastavka") return;
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <Link href="/create-order?type=Dastavka">
      ...
      <h2 className="text-xl font-bold mb-4">Dastavka zakazlari</h2>
